...
class VL_Account_Settings {
	...
	public static function defaults() {
		return array(
			...
			'ask_name'            => 1,       // Спрашивать имя рядом с телефоном.
			'login_from_name'     => 1,       // Логин нового аккаунта — из имени, а не из номера.
			...
			'crm_api_url'          => '',       // Свой адрес CRM (пусто — берём из настроек Simla).
			'crm_api_key'          => '',       // Свой ключ API: только чтение.
			'crm_api_site'         => '',       // Код магазина для своего ключа.
			'crm_api_log'          => 0,        // Писать каждый запрос к CRM в журнал.
			...
			'crm_welcome_bonus'    => 1000,     // Приветственные баллы: по ним узнаём «пустой» счёт при дублях.
			...
			'crm_cart_hint'        => 1,        // Показывать администратору, почему нет поля списания.
			'crm_fix_coupon_email' => 1,        // Чинить купоны с ограничением на технический e-mail.

			// Сопоставление и объединение аккаунтов.
			'identity_orders'      => 1,        // Искать аккаунт по телефону в заказах сайта.
			'identity_crm'         => 1,        // Искать покупателя по телефону в RetailCRM.
			'identity_merge'       => 1,        // Объединять дубли автоматически.
			'identity_delete_merged' => 1,      // Удалять пустой аккаунт после объединения.
			...
			'crm_orders_history'   => 1,        // Показывать в кабинете заказы из CRM, которых нет на сайте.
