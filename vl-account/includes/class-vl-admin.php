	...
	protected function render_orders( $s ) {
		...
		<table class="form-table" role="presentation">
			...
			$this->checkbox(
				...
				__( 'Аккаунт заведён по SMS, своей почты у него ещё нет: адрес, который покупатель ввёл в своём же оформлении заказа, записывается сразу и уезжает в CRM. Подтверждать нечего — он вошёл по коду и ввёл адрес сам. Смена уже привязанного адреса по-прежнему идёт через письмо.', 'vl-account' )
			);
