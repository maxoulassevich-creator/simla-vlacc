...
class VL_Account_Admin {
	...
	protected function render_forms( $s ) {
		...
		<table class="form-table" role="presentation">
			...
			$this->checkbox( 'login_from_name', $s, __( 'Логин из имени', 'vl-account' ), __( 'У нового аккаунта «Имя пользователя» будет вида <code>aleksandr</code>, а не номер телефона. Логин техничен: покупатель входит по номеру и коду, а не по нему. У уже созданных аккаунтов логин не меняется — WordPress этого не позволяет.', 'vl-account' ) );
	...
	protected function render_account( $s ) {
		...
		<table class="form-table" role="presentation">
			...
			$this->select(
				'ws_size_styles',
				$s,
				__( 'Оформление кнопок', 'vl-account' ),
				array(
					'own'   => __( 'Плагина: белые квадраты, выбранный — тёмный', 'vl-account' ),
					'theme' => __( 'Темы: свои стили для .size-button', 'vl-account' ),
				),
				__( 'Кнопки размеров стоят внутри формы товара, поэтому правила темы для кнопок покупки красят их в красный во всю ширину. Оформление плагина это перебивает. Режим «темы» нужен, только если в теме есть собственные стили для <code>.size-button</code>.', 'vl-account' )
			);
			$this->text(
				'ws_size_attributes',
				$s,
	...
	protected function render_orders( $s ) {
		...
		<table class="form-table" role="presentation">
			...
			$this->checkbox(
				'email_from_checkout',
				$s,
				__( 'Первый адрес из заказа — без подтверждения', 'vl-account' ),
				__( 'Аккаунт заведён по SMS, своей почты у него ещё нет: адрес из его же оформления заказа записывается сразу и уезжает в CRM. Сразу — только если адрес ничей: нет аккаунта с ним, нет чужих гостевых заказов и нет карточки CRM с другим номером. Иначе к адресу привязана чужая история, и он уходит на подтверждение по ссылке. Смена уже привязанного адреса — всегда через письмо.', 'vl-account' )
			);
	...
	protected function render_crm( $s ) {
		...
		<table class="form-table" role="presentation">
			...
			$this->text(
				'crm_welcome_bonus',
				$s,
				__( 'Приветственные баллы', 'vl-account' ),
				__( 'Сколько баллов CRM начисляет за вступление в программу. Нужно на случай дублей карточек: если на одном счёте ровно эта сумма, а на другом — своя, кабинет покажет счёт с настоящим балансом покупателя.', 'vl-account' ),
				'number'
			);
			?>
		</table>
		...
		<table class="form-table" role="presentation">
			...
			$this->checkbox(
				'crm_orders_history',
				$s,
				__( 'Заказы из CRM в кабинете', 'vl-account' ),
				__( 'В разделе «Мои заказы» отдельным списком показываются покупки, оформленные не на сайте: в магазине, по телефону или через менеджера. Только для чтения.', 'vl-account' )
			);
			$this->checkbox(
				'crm_sync_daily',
				$s,
